                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            <div class="float-right">
                                ...
                                <a href="{{route('tarefa.exportacao')}}" target="_blank">PDF V2</a>
                        ...
                        <tbody>
                            @foreach($tarefas as $key => $t)
                                <tr>
                                    ...
                                    <td>{{ ucwords($t['tarefa']) }}</td>
                                    <td>{{ date('d/m/Y', strtotime($t['data_limite_conclusao'])) }}</td>
                                    <td><a href="{{ route('tarefa.edit', $t['id']) }}" class="btn btn-outline-primary">Editar</a></td>
                                    <td>
                                        <form id="form_{{$t['id']}}" method="post" action="{{ route('tarefa.destroy', ['tarefa' => $t['id']]) }}">
                                            @method('DELETE')
                                            @csrf
                                        </form>
                                        <a href="#" class="btn btn-outline-danger" onclick="document.getElementById('form_{{$t['id']}}').submit()">Excluir</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
